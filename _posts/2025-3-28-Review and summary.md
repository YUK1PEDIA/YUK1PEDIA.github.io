...
8. Spring Security & RBAC
   - Spring Security 基于一系列过滤器，就像银行的一道道关卡，队进入应用的每个请求进行检查
   ...
   - RBAC：Role-Based Access Control，基于角色的访问控制，通过 **角色** 来管理用户对资源的访问权限，**Ruoyi 脚手架采用的权限控制模型就是 RBAC**。RBAC 模型的核心是将用户和权限解耦，通过角色作为中间桥梁来关联两者
